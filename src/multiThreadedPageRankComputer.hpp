...
private:
    ...
    void generateIds(Network const& network) const {
        ...
        auto beginning = pages.begin();
        auto end = pages.begin() + pagesPerThread;

        for (uint32_t i = 1; i <= numThreads; ++i) {
            threads.emplace_back(std::thread(&MultiThreadedPageRankComputer::generateIdsThread, this, beginning, end, network));
            beginning = end;
            end += pagesPerThread;
            if (i == numThreads - 1) {
                end = pages.end(); // Last thread may have more pages than the rest
            }
        }
    ...
    void generateIdsThread(std::vector<Page>::iterator beginning, std::vector<Page>::iterator end,
